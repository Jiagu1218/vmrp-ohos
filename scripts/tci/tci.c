...
/* OHOS TCI port (5.0.1 / ARM64 host): no ffi.inc.c.
 * We use the standard helper_function fat-cast below (the #else branch of
 * INDEX_op_call). 64-bit host ABI passes all helper args in registers, so the
 * wasm32-specific do_op_call / global_helper_table machinery is unneeded. */

/* Used by error reporting (GETPC) when CONFIG_DEBUG_TCG. Single ARM target,
 * so no per-arch #define tci_tb_ptr rename is needed (unlike FUTO's multi-arch
 * emscripten build). */
#if defined(GETPC)
