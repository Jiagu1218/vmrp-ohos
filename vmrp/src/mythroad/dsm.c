...
/* OHOS_IMAGE_DECODE: 鸿蒙原生 Image C API 实现的 platEx 图片接口。
 * 独立编译单元(ohos_src/ohos_image_decode.c)，不依赖 vmrp 内部头文件。
 * 非 OHOS 构建时此文件不被链接，extern 函数无定义，不会调用。 */
#ifdef __OHOS__
extern int32_t ohos_image_get_info(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_image_decode(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_image_display_lcd(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_image_draw_direct(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_gif_decode(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_gif_release(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_gif_display(uint8_t *input, int32_t input_len, uint8_t **output, int32_t *output_len);
extern int32_t ohos_gif_stop(int32_t handle);
#endif
...
/* 宿主写入最新加速度数据。
 * 仅缓存 x/y/z，不投递 mr_event——由 worker 线程在消费
 * API_CMD_MOTION 命令时调用 dsm_dispatch_motion_event() 投递，
 * 避免 Unicorn 并发访问导致 SIGBUS。
...
/* 获取已缓存的加速度数据，供 mythroad.c 写入 Unicorn 内存。 */
...
/* 由 worker 线程调用：若 motion_enabled 则投递 MR_MOTION_EVENT。
 * worker 线程保证串行执行（与 timer/event 同一线程），Unicorn 安全。
 *
 * param2 必须是 Unicorn ARM 可见的 32 位虚拟地址，不能是宿主
 * &motion_acc 的 64 位地址（截断后 ARM 代码解引用会 SIGBUS）。
 * dsm_write_motion_acc_to_arm() 将数据写入 Unicorn 内存并返回
 * 该 32 位地址。 */
...
int32 mr_plat(int32 code, int32 param) {
    switch (code) {
        ...
        case 4002:  // 给动感芯片上电
            motion_powered = 1;
            /* 通知宿主开启传感器订阅，由 vmrp_api_motion_power 回调实现 */
            { extern void vmrp_api_motion_power(int on); vmrp_api_motion_power(1); }
            return MR_SUCCESS;
        case 4003:  // 给动感芯片断电
            motion_powered = 0;
            motion_enabled = 0;
            /* 通知宿主停止传感器订阅，由 vmrp_api_motion_power 回调实现 */
            { extern void vmrp_api_motion_power(int on); vmrp_api_motion_power(0); }
            return MR_SUCCESS;
        case 4004:  // 动感芯片监听晃动模式
            motion_enabled = 1;
            motion_mode = MR_MOTION_EVENT_SHAKE;
            return MR_SUCCESS;
        case 4005:  // 动感芯片监听倾斜模式
            motion_enabled = 1;
            motion_mode = MR_MOTION_EVENT_TILT;
            return MR_SUCCESS;
        case 4006:  // 获取动感芯片数值范围
            /* 返回值 >1000 表示范围 ±(ret-1000)。
             * HarmonyOS 加速度传感器单位 m/s²，转换后 mG，
             * 典型范围 ±2g → ±2000mG，故返回 1000+2000=3000。 */
            return 3000;
        case 3012:  /* 停止 GIF 动画，param=mr_platEx(3011)返回的句柄 */
#ifdef __OHOS__
            if (ohos_gif_stop(param) == 0)
                return MR_SUCCESS;
#endif
            return MR_IGNORE;
        default:
...
int32 mr_platEx(int32 code, uint8 *input, int32 input_len, uint8 **output, int32 *output_len, MR_PLAT_EX_CB *cb) {
    ...
    switch (code) {
        /* OHOS_MEM_EXT: platEx(1001) 获取屏幕缓冲(第二内存)。
         * ARM ext 模式下在 arm_ext_executor.c 的 table[38] 拦截,
         * 这里只处理非 ext 路径(Lua 等)的调用。 */
        case 1001: {
            /* OHOS_MEM_EXT: platEx(1001) 获取屏幕缓冲(第二内存)。
             * ARM ext 模式下在 arm_ext_executor.c 的 table[38] 拦截,
             * 这里只处理非 ext 路径(Lua 等)的调用。
             * 屏幕缓冲大小: SCREEN_WIDTH*SCREEN_HEIGHT*2(RGB565)。 */
        ...
        case 1002: {
            /* 释放第二内存, input 是 1001 返回的地址 */
            if (input) free(input);
            return MR_SUCCESS;
        }
        case 1012: {  //申请内部cache
            /* OHOS_MEM_EXT: 申请内部 RAM, input_len 是请求大小 */
            if (input_len > 0) {
        ...
        case 1013: {  //释放内部cache
            /* OHOS_MEM_EXT: 释放内部 RAM, input 是 1012 返回的地址 */
            if (input) free(input);
            return MR_SUCCESS;
        }
        case 1014: {  //申请拓展内存
        ...
        /* OHOS_IMAGE_DECODE: platEx(3001) 获取图片信息。
         * input=MRAPP_IMAGE_ORIGIN_T, output=MRAPP_IMAGE_SIZE_T。
         * 鸿蒙原生 OH_ImageSourceNative 实现；非 OHOS 构建走 MR_IGNORE。
         * SRC_NAME 模式: 先用 mr_open/read 读取文件到内存，再以 SRC_STREAM 传给解码。 */
        case 3001: {
#ifdef __OHOS__
            {
                uint8_t *_img_buf = NULL;
                int32_t _img_buf_len = 0;
                if (input && input_len >= 12) {
                    int32_t _st = 0;
                    memcpy(&_st, input + 8, 4);
                    if (_st == 0) {
                        char *_fname = NULL;
                        memcpy(&_fname, input + 0, 4);
                        int32 _fd = mr_open(_fname, MR_FILE_RDONLY);
                        if (_fd > 0) {
                            mr_seek(_fd, 0, 2);
                            _img_buf_len = mr_seek(_fd, 0, 1);
                            mr_seek(_fd, 0, 0);
                            if (_img_buf_len > 0 && _img_buf_len < 16*1024*1024) {
                                _img_buf = (uint8_t*)calloc(1, _img_buf_len);
                                if (_img_buf) mr_read(_fd, _img_buf, _img_buf_len);
                            }
                            mr_close(_fd);
                        }
                        if (_img_buf) {
                            ...
                            memcpy(_mod, input, 8);
                            int32_t _one = 1;
                            memcpy(_mod + 8, &_one, 4);
                            char *_datap = (char*)_img_buf;
                            int32_t _dlen = _img_buf_len;
                            memcpy(_mod + 0, &_datap, 4);
                            memcpy(_mod + 4, &_dlen, 4);
                            if (ohos_image_get_info(_mod, 12, output, output_len) == 0) {
                                free(_img_buf);
                                return MR_SUCCESS;
                            }
                            free(_img_buf);
                        ...
                        if (ohos_image_get_info(input, input_len, output, output_len) == 0)
                            return MR_SUCCESS;
                    }
                }
            }
        ...
        /* OHOS_IMAGE_DECODE: platEx(3002) 图片解码→RGB565。
         * input=MRAPP_IMAGE_DECODE_T, output=解码后像素数据。
         * 鸿蒙原生 OH_ImageSourceNative+OH_PixelmapNative 实现；
         * 支持 RGB565 直出，fallback RGBA8888→RGB565 转换。
         * SRC_NAME: 同 3001 预处理，读取文件到内存后以 SRC_STREAM 解码。 */
        case 3002: {
#ifdef __OHOS__
            {
                uint8_t *_img_buf = NULL;
                ...
                if (input && input_len >= 24) {
                    ...
                    if (_st == 0) {
                        ...
                        if (_img_buf) {
                            uint8_t _mod[24];
                            memcpy(_mod, input, 24);
                            int32_t _one = 1;
                            memcpy(_mod + 16, &_one, 4);
                            char *_datap = (char*)_img_buf;
                            int32_t _dlen = _img_buf_len;
                            memcpy(_mod + 0, &_datap, 4);
                            memcpy(_mod + 4, &_dlen, 4);
                            if (ohos_image_decode(_mod, 24, output, output_len) == 0) {
                                free(_img_buf);
                                return MR_SUCCESS;
                            }
                            free(_img_buf);
                        ...
                        if (ohos_image_decode(input, input_len, output, output_len) == 0)
                            return MR_SUCCESS;
                    }
                }
            }
        ...
        /* platEx(3003) MR_DECODE_STATUS: SkyEngine 自身标记"未实现"，跳过 */
        /* OHOS_IMAGE_DECODE: platEx(3004) MR_GIF_DECODE——解码 GIF 动图所有帧。
         * input=MRAPP_IMAGE_DECODE_T, output=MRAPP_GIF_HEADER*(含帧链表)。
         * 鸿蒙原生 OH_ImageSourceNative 多帧解码 API。
         * SRC_NAME: 读取文件到内存后以 SRC_STREAM 处理。 */
        case 3004: {
#ifdef __OHOS__
            {
                uint8_t *_img_buf = NULL;
                ...
                if (input && input_len >= 24) {
                    ...
                    if (_st == 0) {
                        ...
                        if (_img_buf) {
                            ...
                            memcpy(_mod + 0, &_datap, 4);
                            memcpy(_mod + 4, &_dlen, 4);
                            if (ohos_gif_decode(_mod, 24, output, output_len) == 0) {
                                free(_img_buf);
                                return MR_SUCCESS;
                            }
                            free(_img_buf);
                        ...
                        if (ohos_gif_decode(input, input_len, output, output_len) == 0)
                            return MR_SUCCESS;
                    }
                }
            }
        ...
        /* OHOS_IMAGE_DECODE: platEx(3005) MR_GIF_RELEASE——释放 GIF 解码资源。
         * input=MRAPP_GIF_HEADER*(3004 返回的指针)。 */
        case 3005: {
#ifdef __OHOS__
            if (input && ohos_gif_release(input, input_len, output, output_len) == 0)
                return MR_SUCCESS;
#endif
            return MR_IGNORE;
        }
        /* platEx(3007) MR_DRAW_BUFFER: MTK GDI 图层 blt 到屏幕。
         * vmrp 的 mr_screenBuf 已由渲染循环自动刷新，3007 为空操作。
         * MRP 应用几乎不直接调用此接口（用 3009 DMA 刷屏代替）。 */
        case 3007:
#ifdef __OHOS__
            return MR_SUCCESS;
#else
            return MR_IGNORE;
#endif
        /* platEx(3008) MR_GET_ACT_LAYER: 返回 MTK GDI gdi_layer_struct。
         * vmrp 无 MTK GDI 层，无法提供有意义的结构体。
         * MRP 应用几乎不调用此接口，返回 MR_IGNORE。 */
        case 3008:
            return MR_IGNORE;
        /* OHOS_IMAGE_DECODE: platEx(3011) 显示 GIF 动画（自动播放到屏幕）。
         * input=T_DRAW_DIRECT_REQ{src,src_len,src_type,ox,oy,w,h},
         * output=T_DSM_COMMON_RSP{p1=动画句柄}。
         * 内部解码 GIF 后由 ohos_gif_tick() 逐帧绘制。
         * SRC_NAME: 读取文件到内存后以 SRC_STREAM 处理。 */
        case 3011: {
#ifdef __OHOS__
            {
                ...
                if (input && input_len >= 28) {
                    ...
                    if (_st == 0) {
                        ...
                        if (_img_buf) {
                            ...
                            memcpy(_mod + 0, &_datap, 4);
                            memcpy(_mod + 4, &_dlen, 4);
                            if (ohos_gif_display(_mod, 28, output, output_len) == 0) {
                                free(_img_buf);
                                return MR_SUCCESS;
                            }
                            free(_img_buf);
                        ...
                        if (ohos_gif_display(input, input_len, output, output_len) == 0)
                            return MR_SUCCESS;
                    }
                }
            }
        ...
        /* OHOS_IMAGE_DECODE: platEx(3009) DMA 刷屏。
         * input={src_ptr,src_len,src_type,ox,oy,w,h}，
         * 将解码后像素直接绘制到 mr_screenBuf。
         * ARM ext 模式下 table[38] 桥接自动完成 host→ARM 转写。
         * SRC_NAME: 读取文件到内存后以 SRC_STREAM 处理。 */
        case 3009: {
#ifdef __OHOS__
            {
                ...
                if (input && input_len >= 28) {
                    ...
                    if (_st == 0) {
                        ...
                        if (_img_buf) {
                            ...
                            memcpy(_mod + 0, &_datap, 4);
                            memcpy(_mod + 4, &_dlen, 4);
                            if (ohos_image_display_lcd(_mod, 28, output, output_len) == 0) {
                                free(_img_buf);
                                return MR_SUCCESS;
                            }
                            free(_img_buf);
                        ...
                        if (ohos_image_display_lcd(input, input_len, output, output_len) == 0)
                            return MR_SUCCESS;
                    }
                }
            }
        ...
        /* OHOS_IMAGE_DECODE: platEx(3010) 直接绘制图片（解码+绘制一步完成）。
         * input=T_DRAW_DIRECT_REQ{src,src_len,src_type,ox,oy,w,h}，
         * 解码图片并将指定区域绘制到 mr_screenBuf。
         * SRC_NAME: 读取文件到内存后以 SRC_STREAM 处理。 */
        case 3010: {
#ifdef __OHOS__
            {
                ...
                if (input && input_len >= 28) {
                    ...
                    if (_st == 0) {
                        ...
                        if (_img_buf) {
                            ...
                            memcpy(_mod + 0, &_datap, 4);
                            memcpy(_mod + 4, &_dlen, 4);
                            if (ohos_image_draw_direct(_mod, 28, output, output_len) == 0) {
                                free(_img_buf);
                                return MR_SUCCESS;
                            }
                            free(_img_buf);
                        ...
                        if (ohos_image_draw_direct(input, input_len, output, output_len) == 0)
                            return MR_SUCCESS;
                    }
                }
            }
