...
int32 my_connect(int32 s, int32 ip, uint16 port, int32 type) {
    ...
    // OHOS pay 虚拟 socket: 127.0.0.1:18088 不建立真实 TCP 连接
    // MRP 客户端(netpay.mrp)通过 rop.skymobiapp.com 发起 PREREG/REG/PROP 请求,
    // DNS 映射将此域名指向 127.0.0.1:18088,此处拦截并虚拟化,无需监听端口
    // ip 是主机字节序(与 my_connectSync 中 htonl(ip) 一致),127.0.0.1 = 0x7F000001
    if (ip == 0x7F000001 && port == 18088) {
        printf("my_connect: pay virtual socket (127.0.0.1:18088), no real connect\n");
        // 关闭 my_socket() 分配但不会使用的真实 fd
        CLOSE_SOCKET(data->s);
        data->s = -1;
        data->isPayVirtual = 1;
...
int32 my_closeSocket(int32 s) {
    ...
    int32_t wasPayVirtual = data->isPayVirtual;
    // OHOS pay 虚拟 socket 清理
    ...
    if (wasPayVirtual) {
        // 虚拟 socket 没有真实 fd,跳过 shutdown/close
        return MR_SUCCESS;
    }
...
int32 my_send(int32 s, const char* buf, int len) {
    ...
    // OHOS pay 虚拟 socket: 收集发送数据到缓冲区
    // my_recv 时会解析 HTTP body 中的 TLV 请求并生成响应
    if (data->isPayVirtual) {
        printf("[my_send] pay virtual: appending %d bytes\n", len);
        if (data->paySendLen + len > data->paySendCap) {
            int32_t newCap = (data->paySendLen + len) * 2;
            uint8_t* newBuf = (uint8_t*)realloc(data->paySendBuf, newCap);
            if (!newBuf) return MR_FAILED;
            data->paySendBuf = newBuf;
            data->paySendCap = newCap;
        }
    ...
    if (data->cmwapMode) {  // cmwap模式需要通过代理，这里模拟代理的功能
        ...
        if (data->realState == MR_WAITING) {
            ...
            if (data->sendCounter == 1) {  // 第一次发送数据，尝试连接
                ...
                data->realState = MR_SUCCESS;
                if (data->isPayVirtual) {
                    printf("[my_send] pay virtual: cmwap connect done, appending %d bytes\n", len);
                    if (data->paySendLen + len > data->paySendCap) {
                        int32_t newCap = (data->paySendLen + len) * 2;
                        uint8_t* newBuf = (uint8_t*)realloc(data->paySendBuf, newCap);
                        if (!newBuf) return MR_FAILED;
...
int32 my_recv(int32 s, char* buf, int len) {
    ...
    if (data->isPayVirtual) {
        if (!data->payRecvBuf) {
            ...
            int32_t bodyOff = 0;
            if (data->paySendBuf) {
                for (int32_t i = 0; i < data->paySendLen - 3; i++) {
                    if (data->paySendBuf[i] == '\r' && data->paySendBuf[i+1] == '\n' &&
                        data->paySendBuf[i+2] == '\r' && data->paySendBuf[i+3] == '\n') {
                        bodyOff = i + 4;
                        break;
                    }
                }
            ...
            memcpy(data->payRecvBuf + headLen, tlvBuf, tlvLen);
            data->payRecvLen = totalRespLen;
            data->payRecvOff = 0;
            // 释放发送缓冲区
            free(data->paySendBuf);
            data->paySendBuf = NULL;
