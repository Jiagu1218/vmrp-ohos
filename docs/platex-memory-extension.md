...
> **注意**: 上游重构后 `table[38]` 拆为 `aex_t038()` 函数在 `aex_table.c`（line 2023），
> 但 CMake patch 的搜索锚点和缩进与旧 `arm_ext_executor.c` 一致，补丁仍正确匹配。
...
### 1. `vmrp/src/include/arm_ext_internal.h` — 新增字段（CMake patch 注入）

上游 `ArmExtModule` 结构体无 OHOS 特有字段，由 `scripts/CMakeLists.txt` 在构建时注入：

```c
uint32_t screen_cap;          // 上游已有
/* OHOS_PLATEX_SCREEN_BUF: platEx(1001) 分配的离屏渲染缓冲 */
uint32_t screen_buf_addr;     // CMake 注入，插在 screen_cap 之后
uint32_t screen_buf_len;      // CMake 注入
/* ARM 可见 mr_screen_w/h... */  // 上游已有

uint32_t exram_addr;          // 上游已有 (1014 exRAM)
uint32_t exram_len;           // 上游已有
/* OHOS_PLATEX_IRAM: platEx(1012) 分配的内部 cache 缓冲 */
uint32_t iram_addr;           // CMake 注入，插在 exram_len 之后
uint32_t iram_len;            // CMake 注入
uint32_t internal_table_addr; // 上游已有
```

文件在 `:restore_patched` 列表中，构建后还原为上游原始状态（无 OHOS 字段）。

### 2. `scripts/CMakeLists.txt` — CMake patch (OHOS_PLATEX_MEM_EXT)

在 `aex_table.c` 的 `aex_t038()` 函数中，于 1014 拦截块（`MR_MALLOC_SCRRAM` 注释）之前插入 1001/1002/1012/1013 拦截：
...
### 3. `vmrp/src/mythroad/dsm.c` — host 路径实现（CMake patch 注入）

上游 `dsm.c` 的 `mr_platEx()` 中只有 case 1012/1013（返回 `MR_IGNORE`），无 case 1001/1002。
由 `scripts/CMakeLists.txt` 的 `OHOS_PLATEX_DSM` patch 在构建时注入 case 1001/1002/1012/1013 实现：
...
文件在 `:restore_patched` 列表中，构建后还原为上游原始状态（仅 MR_IGNORE）。
