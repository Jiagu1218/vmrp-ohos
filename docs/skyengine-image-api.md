...
vmrp/src/mythroad/dsm.c    # CMake 注入 case 3001/3002/3004/3005/3007/3008/3009/3010/3011 + mr_plat case 3012
...
scripts/CMakeLists.txt     # 注入 ohos_image_decode.cpp + __OHOS__ 宏 + 链接 image_source/pixelmap
                             + 注入 dsm.c case 分支 (OHOS_IMAGE_DSM)
```
...
dsm.c switch(code) case 3001~3011
    └─ CMake patch 注入的 case 分支 → 调 ohos_image_* / ohos_gif_* 函数
```

> **注意**: dsm.c 中的 case 3001~3011 代码并非上游自带，而是 `scripts/CMakeLists.txt` 的
> `OHOS_IMAGE_DSM` patch 在构建时注入。上游 dsm.c 无任何 `__OHOS__` 代码块。
> `__OHOS__` 宏定义在 vmrp-shared target 上（line 668），使注入的 case 分支生效。
...
鸿蒙原生 Image C API 只接受内存数据（`OH_ImageSourceNative_CreateFromData`），不接受文件名。因此 dsm.c 中 CMake 注入的 case 分支在调用 ohos_image_* 前做预处理：
...
| Patch ID | 文件 | 作用 |
|----------|------|------|
| `OHOS_IMAGE_DSM` | dsm.c | 注入 case 3001~3011 分支（上游无此代码） |
| `__OHOS__` macro | vmrp-shared target | 使 CMake 注入的 dsm.c case 分支条件编译生效 |
| `OHOS_GIF_TICK` | mythroad.c | mr_timer 末尾注入 ohos_gif_tick() + dsm_dispatch_motion_event() 调用 |
